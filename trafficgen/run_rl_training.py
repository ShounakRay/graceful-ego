...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wandb", action="store_true", help="Whether to upload log to wandb."
    )
    parser.add_argument(
        "--case_num_train",
        default=1385,
    ...
    assert os.path.isdir(data_folder_train), (
        "Can't find {}. ".format(data_folder_train) + HELP
    )
    ...
    assert os.path.isdir(data_folder_test), (
        "Can't find "
        + data_folder_test
        + ". It seems that you don't download the validation data. "
