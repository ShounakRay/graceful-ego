...
def process_case_to_input(case, agent_range=60):
    ...
    range_mask = (abs(agent[:, 0] - 40 / 50) < agent_range / 50) * (
        abs(agent[:, 1]) < agent_range / 50
    )
...
def get_type_class(line_type):
    from metadrive.scenario.scenario_description import (
        ScenarioDescription as SD,
        MetaDriveType,
    )

    # NOTE: [SR] Changed from `LineType` to `PGLineType`
    from metadrive.constants import PGLineType

    if line_type in range(1, 4):
        # NOTE: [SR] LANE_CENTER_LINE didn't exist in `metadrive.type.py`, so I added it.
...
def save_as_metadrive_data(index, pred_i, other, save_path):
    ...
    scenario[SD.METADATA][SD.TIMESTEP] = np.array(
        [x / 10 for x in range(190)], dtype=np.float32
    )
    ...
    for i in range(num_vehicles):
        ...
        agent_state["state"]["size"] = (
            np.ones([len(agent_i), 2], dtype=np.float32) * 2.332
        )
        agent_state["state"]["size"][:, 0] *= 5.286  # length
        agent_state["state"]["size"][:, 1] *= 2.332  # width

        agent_state["state"]["heading"] = (
            agent_i[:, 4].reshape(-1, 1).astype(np.float32)
        )
    ...
    for step_count, step_state in enumerate(traffic_light_compact_state):
        ...
        for traffic_light_state in step_state:
            ...
            if lane_id not in scenario[SD.DYNAMIC_MAP_STATES]:
                scenario[SD.DYNAMIC_MAP_STATES][
                    lane_id
                ] = _traffic_light_state_template(lane_id, track_len)
            scenario[SD.DYNAMIC_MAP_STATES][lane_id]["state"]["lane"][
                step_count
            ] = traffic_light_state[0]
            scenario[SD.DYNAMIC_MAP_STATES][lane_id]["state"]["stop_point"][
                step_count
            ] = traffic_light_state[1:4]
            scenario[SD.DYNAMIC_MAP_STATES][lane_id]["state"]["object_state"][
                step_count
...
def get_vec_based_rep(case_info):
    ...
    cent_mask = np.repeat(
        case_info["center_mask"][:, np.newaxis], axis=1, repeats=agent_num
    )
    ...
    vec_len = np.clip(
        np.sqrt(np.square(y2 - y1) + np.square(x1 - x2)), a_min=4.5, a_max=5.5
    )
...
def _process_map_inp(case_info):
    ...
    case_info["lane_mask"] = np.concatenate(
        [
            case_info["center_mask"],
            case_info["bound_mask"],
            case_info["cross_mask"],
            case_info["rest_mask"],
        ],
        axis=1,
    )
...
def process_data_to_internal_format(data):
    ...
    case_info["agent"], case_info["agent_mask"] = process_agent(
        data["all_agent"], False
    )
    (
        case_info["center"],
        case_info["center_mask"],
        ...
        case_info["rest"],
        case_info["rest_mask"],
    ) = process_map(data["lane"], data["traffic_light"], lane_range=RANGE, offest=0)
