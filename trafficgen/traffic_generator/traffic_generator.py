...
from trafficgen.traffic_generator.utils.data_utils import (
    InitDataset,
    save_as_metadrive_data,
    from_list_to_batch,
    transform_to_agent,
    process_case_to_input,
)
...
# NOTE: As per the main README, we must download the TWO checkpoint files (init, act) from the following link and put them in the ckpt folder: "trafficgen/traffic_generator/ckpt".
...
class TrafficGen:
    def __init__(self, cfg):
        ...
        """
        NOTE: [SR] VERY IMPORTANT CHANGE (Especially for MacOS users without CUDA support):
        From official `load_from_checkpoint` documentation:
        "map_location:
            If your checkpoint saved a GPU model and you now load on CPUs
            or a different number of GPUs, use this to map to the new setup.
            The behaviour is the same as in :func:`torch.load`."
        Turns out that the checkpoint files were saved with CUDA support (since we receive an error if we don't specify
        `map_location`), so we need to EXPLICITY map the model to the CPU.
        """
        ...
        self.data_loader = DataLoader(
            init_dataset, shuffle=False, batch_size=1, num_workers=0
        )
    ...
    def place_vehicles_for_single_scenario(
        self, batch, index=None, vis=False, vis_dir=None, context_num=1
    ):
        self.wash(batch)
    ...
    def place_vehicles(self, vis=True):
        ...
        with torch.no_grad():
            for idx, data in enumerate(tqdm(self.data_loader)):
                ...
                model_output = self.place_vehicles_for_single_scenario(
                    batch, idx, vis, init_vis_dir, context_num
                )
                ...
                output["unsampled_lane"] = (
                    batch["other"]["unsampled_lane"][0].cpu().numpy()
                )
                output["traf"] = self.data_loader.dataset[idx]["other"]["traf"]
                output["gt_agent"] = batch["other"]["gt_agent"][0].cpu().numpy()
                output["gt_agent_mask"] = (
                    batch["other"]["gt_agent_mask"][0].cpu().numpy()
                )
    ...
    def generate_traj(self, snapshot=True, gif=False, save_metadrive=False):
        ...
        with torch.no_grad():
            for i in tqdm(range(self.cfg["data_usage"])):
                with open(
                    f"traffic_generator/output/initialized_tmp/{i}.pkl", "rb+"
                ) as f:
                    data = pickle.load(f)
                ...
                if gif:
                    ...
                    if (
                        agent.shape[1] > 2
                    ):  # PZH: I don't know what this means here. Just add this if to avoid error.
        ...
        if snapshot:
            print(
                "Trajectory visualization has been generated to vis/snapshots folder."
            )
    ...
    def inference_control(self, data, ego_gt=True, length=190, per_time=20):
        ...
        for i in range(0, length - 1, per_time):
            ...
            for j in range(agent_num):
                a_case = {}
                a_case["agent"], a_case["lane"] = transform_to_agent(
                    current_agent[j], current_agent, data["lane"]
                )
            ...
            all_pred = torch.cat(
                [pos_pred, velo_pred, heading_pred.unsqueeze(-1)], dim=-1
            )

            best_pred_idx = torch.argmax(prob, dim=-1)
            best_pred_idx = best_pred_idx.view(agent_num, 1, 1, 1).repeat(
                1, 1, *all_pred.shape[2:]
            )
            best_pred = (
                torch.gather(all_pred, dim=1, index=best_pred_idx)
                .squeeze(1)
                .cpu()
                .numpy()
            )
            ...
            for j in range(start_idx, agent_num):
                ...
                pred_agent[i + 1 : i + per_time + 1, j, :2] = copy.deepcopy(
                    pos[:pad_len]
                )
                pred_agent[i + 1 : i + per_time + 1, j, 2:4] = copy.deepcopy(
                    vel[:pad_len]
                )
                pred_agent[i + 1 : i + per_time + 1, j, 4] = copy.deepcopy(
                    heading[:pad_len]
                )
